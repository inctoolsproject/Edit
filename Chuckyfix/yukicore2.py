...
cl = LINETCR.LINE()
cl.login(token="EpuD9LuSF9g4YzDRSSEa.l6Ha5iuE5BX8ACSSNwEkQG.ZaiKUWv/EAFKP/46WYtJo+8hAs9no5ytTOAN/ZKzBEY=")
cl.loginResult()
...
Bots=[mid]
admin = ["u2fa840e4851ee5bc7c69cb2c9d678aaa"]
wait = {
    "likeOn":True,
    "alwayRead":False,
    "detectMention":True,    
...
def bot(op):
    try:
                    ...
                    if msg.text is not None:
                        ...
                        if data['status'] == 200:
                            if data['result']['result'] == 100:
                                cl.sendText(msg.to, "[Chatbot simi] " + data['result']['response'].encode('utf-8'))

        if op.type == 26:
            msg = op.message
            if msg.from_ in lock["target"] and lock["status"] == True and lock["target"][msg.from_] == True:
                text = msg.text
                if text is not None:
                    cl.sendText(msg.to,"PLEASE ,  I SAID THE FUCK UP..")
                    cl.kickoutFromGroup(msg.to,[msg.from_])
                if msg.contentType == 7:
                    cl.sendText(msg.to,"PLEASE , I SAD THE FUCK UP..")
                    cl.kickoutFromGroup(msg.to,[msg.from_])
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     ...
                     balas = [" I'm bussy , please leave a message ."]
                     ret_ = "[Reply mesage] " + random.choice(balas)
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["kickMention"] == True:
                     contact = cl.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["DONT TAG ME BITCH !!"]
                     ret_ = " KICK MENTION !!! " + random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
                     for mention in mentionees:
            ...
            if wait["wordban"] == True:
                if msg.text in ["Kick",".","kick","kickall","Kickall","Nuke",".kickall",".Kickall",".salkenall","byr","Nuklir",".Nuklir","Mayhem","Purge","purge","Fkick","Vkick","~"]:
                  if msg.from_ not in admin or bot:
                    try:
                        cl.sendText(msg.to,"Kicker terdekteksi. Dia bersalah telah menggunakan kata terlarang !!")
                        cl.kickoutFromGroup(msg.to,[msg.from_])
                    except Exception as error:
                        print error
        ...
        if op.type == 25:
            msg = op.message
            if msg.contentType == 7:
               if wait["ceksticker2"] == True:
                msg.contentType = 0
                stk_id = msg.contentMetadata['STKID']
                stk_ver = msg.contentMetadata['STKVER']
                pkg_id = msg.contentMetadata['STKPKGID']
                filler = "==Check Sticker==\nSTKID : %s\nSTKPKGID : %s\nSTKVER : %s\n==Link Sticker==\nline://shop/detail/%s" % (stk_id,pkg_id,stk_ver,pkg_id)
        ...
        if op.type == 25:
            ...
            if msg.contentType == 13:
                ...
                if wait["lang"] == "JP":
                    ...
                    cl.sendText(msg.to,helpmsg)
            elif msg.text in ["Papcontact"]:
                    h = cl.getContact(msg.to)
                    cl.sendImageWithURL(msg.to,"http://dl.profile.line-cdn.net/" + h.pictureStatus)
                    
            elif msg.text in ["Covercontact"]:
                                h = cl.getContact(msg.to)
                                cl.sendImageWithURL(msg.to,"http://dl.profile.line-cdn.net/" + h.pictureStatus)
                            
            elif msg.text in ["Biocontact"]:
                                h = cl.getContact(msg.to)
                                cl.sendText(msg.to,"===[•StatusKamu•]===\n" + h.statusMessage)
            elif "My status" == msg.text:
                eltime = time.time() - mulai
                contactlist = cl.getAllContactIds()
                kontak = cl.getContacts(contactlist)
                gruplist = cl.getGroupIdsJoined()
                gls = cl.getGroups(gruplist)
                ...
                cl.sendText(msg.to,"[Nama]: " + cl.getProfile().displayName + "\n[Runtime]: " + waktu(eltime) + "\n[Friend]: " + str(len(kontak)) + "\n[Group]: " + str(len(gls)) + "\n[Date]: " + rst)            
...
#==============================================================================#
                ...
                if wait["inviteprotect"] == True: md+="InvitationProtect on \n"
                else:md+="InvitationProtect off \n"
                if wait["cancelprotect"] == True: md+="􀠁􀆩􏿿CancelProtect on \n"
                else:md+="CancelProtect off \n"
                if wait["Autokick"] == True: md+= "Autokick on \n"
                else:md+="Autokick off \n"
                if wait["Autorespon"] == True: md+="Autorespon on \n"
                else:md+="Autorespon off \n"
                cl.sendText(msg.to,md)
                msg.contentType = 13
                msg.contentMetadata = {'mid': "uaca55463c423c3632012598148691da7"}
                cl.sendMessage(msg)
              ...
              if msg.from_ in admin:
                if wait["ceksticker2"] == True:
                    ...
                    wait["ceksticker2"]=True
                    cl.sendText(msg.to, "Deteksi sticker sudah aktif")
            elif msg.text.lower() in ["Sticker off","detectsticker:off","sticker off"]:
              if msg.from_ in admin:
                if wait["ceksticker2"] == False:
                    cl.sendText(msg.to, "Deteksi sticker dinonaktifkan")
                else:
                    wait["ceksticker2"]=False
                    cl.sendText(msg.to, "Sticker sudah tidak aktif")
#==============================================================================#
#==============================================================================#
                ...
                wait['alwayRead'] = True
                cl.sendText(msg.to,"Autoread message set to on")
                
            elif msg.text in ["My read off","Read off"]:
                wait['alwayRead'] = False
                cl.sendText(msg.to,"Autoread message set to off")

            elif msg.text in ["Wordban on","wordban on"]:
              if msg.from_ in admin:
                wait["wordban"] = True
                cl.sendText(msg.to,"Wordban Set to on")
                
            elif msg.text in ["Wordban off","wordban off"]:
              if msg.from_ in admin:
                wait["wordban"] = False
                cl.sendText(msg.to,"Wordban Set to off")
                
            elif msg.text in ["My autorespon on","Autorespon:on","My respon on","Respon on"]:
                wait["detectMention"] = True
                cl.sendText(msg.to,"Autorespon set to on")
                
            elif msg.text in ["My autorespon off","Autorespon:off","My respon off","Respon off"]:
                wait["detectMention"] = False
                cl.sendText(msg.to,"Autorespon set to off")
            
            elif msg.text in ["Notag on","Autokick on","Responkick on","Responkick:on"]:
                wait["kickMention"] = True
                cl.sendText(msg.to,"Notag has been set to on")
                
            elif msg.text in ["Notag off","Autokick off","Responkick off","Responkick:off"]:
                wait["kickMention"] = False
                cl.sendText(msg.to,"[Notag has been set to off")
            elif "Talk " in msg.text:
              if msg.from_ in admin:   
                cmd = msg.text.replace("Talk ","")
                if cmd == "on":
                    if lock["status"] == False:
                        lock["status"] = True
                        print "MODE LOCK ACTIVE"
                        cl.sendText(msg.to,"Notalk set to ON")
                    else:
                        cl.sendText(msg.to,"Notalk set to ON")
                elif cmd == "off":
                    if lock["status"] == True:
                        lock["status"] = False
                        print "MODE LOCK NON ACTIVE"
                        cl.sendText(msg.to,"Notalk set to OFF")
                    else:
                        cl.sendText(msg.to,"Notalk set to OFF")
            elif "Namelock on" in msg.text:
              if msg.from_ in admin:
                if msg.to in wait['pname']:
                    cl.sendText(msg.to,"namelock has been on.")
                else:
                    cl.sendText(msg.to,"Namelock has been set on")
              ...
              if msg.from_ in admin:
                if msg.to in wait['pname']:
                    cl.sendText(msg.to,"Namelock turn off.")
                    del wait['pname'][msg.to]
                else:
                    cl.sendText(msg.to,"Namelock set turn off")
            elif ("Notalk " in msg.text):
              if msg.from_ in admin:   
                targets = []
                ...
                for target in targets:
                  if target not in admin:
                    try:
                        lock["target"][target] = True
                        cl.sendText(msg.to,"ᴅᴏ ɴᴏᴛ ᴛʏᴘᴇ ʙᴀsᴛᴀʀᴅs")
                        cl.sendText(msg.to,"sᴜᴋsᴇs ᴀᴅᴅ ᴛᴏ ɴᴏᴛᴀʟᴋ")
                        print "SUCCES LOCK TARGET"
                        break
                    except:
                        cl.sendText(msg.to,"error！")
                        break
              ...
              if msg.from_ in admin:   
                ...
                for target in targets:
                    try:
                        del lock["target"][target]
                        cl.sendText(msg.to,"succes~")
                        print "SUCCES UNLOCK TARGET"
                        break
                    except:
                        cl.sendText(msg.to,"error！")
                        break
...
#==============================================================================#
                ...
                if msg.to in wait2['readPoint']:
                        ...
                        with open('sider.json', 'w') as fp:
                         ...
                         cl.sendText(msg.to,"Lurking set to on")
                else:
                    try:
                            del wait2['readPoint'][msg.to]
                            del wait2['readMember'][msg.to]
                ...
                if msg.to not in wait2['readPoint']:
                    cl.sendText(msg.to,"Check sider telah dinonaktifkan")
                else:
                    try:
                            del wait2['readPoint'][msg.to]
                            del wait2['readMember'][msg.to]
            ...
            elif "Grupcast " in msg.text:
              #if msg.from_ in admin:
                 bctxt = msg.text.replace("Grupcast ", "")
            ...
            elif "GrupcastImage: " in msg.text:
                bc = msg.text.replace("GbroadcastImage: ","")
                gid = cl.getGroupIdsJoined()
                for i in gid:
                    cl.sendImageWithURL(i, bc)
                    
            elif "PmcastImage: " in msg.text:
                bc = msg.text.replace("PmcastImage: ","")
                gid = cl.getAllContactIds()
                for i in gid:
                    cl.sendImageWithURL(i, bc)
                ...
                if cmd == "on":
                    if mimic["status"] == False:
                        mimic["status"] = True
                        cl.sendText(msg.to,"Mimic has been set to on")
                    else:
                        cl.sendText(msg.to,"Sudah on")
                elif cmd == "off":
                    if mimic["status"] == True:
                        mimic["status"] = False
                        cl.sendText(msg.to,"Mimic set to off")
                    else:
            ...
            elif msg.text in ["Papimage",":v","Pap"]:
...
#==============================================================================#
            ...
            elif "search @" in msg.text.lower() and helpmsg.lower() not in msg.text.lower():
                   if 'MENTION' in msg.contentMetadata.keys() != None:
                    ...
                    if ngroup == "":
                        ...
                        cl.sendText(msg.to,"Found In groups:\n%s\n"%(ngroup))
#==============================================================================#
...
#---------------------------------------------------------
                ...
                future = datetime(2018,03,31)
                days = (str(future - today))
                comma = days.find(",")
                days = days[:comma]
                cl.sendText(msg.to,"「About」\n 「yukie」Selfbot edition \n\n「Subscription」\n Expired: " + "\n In days: " + days +  "\n\n「Contact」\n・ LINE: line://ti/p/~yukie2k18")
                msg.contentType = 13
                msg.contentMetadata = {'mid':mid}
                cl.sendMessage(msg)
                ...
                with requests.session() as x:
                    ...
                    b = urllib.quote(a)
                    sam.sendText(msg.to,"「 Searching 」\n" "Type:Search Info\nStatus: Processing")
                    sam.sendText(msg.to, "https://www.twitter.com" + b)
                    sam.sendText(msg.to,"「 Searching 」\n" "Type:Search Info\nStatus: Success")           
...
#==============================================================================#
                    ...
                    try:
                        cl.sendText(msg.to,"Waiting..")
                        cl.sendText(msg.to,"Done restared")
                        restart_program()
                    except:
                        cl.sendText(msg.to,"Please wait")
                        restart_program()
                ...
                cl.sendText(msg.to,"Loading..")
                eltime = time.time() - mulai
                van = "Selfbot sudah berjalan selama " +waktu(eltime)
...
#==============================================================================#
                ...
                location = msg.text.lower().replace("checkpraytime ","")
...
#==============================================#
        if op.type == 17:
            ...
            if wait["welcome"] == True:
	          ...
	          cl.sendText(op.param1,"Selamat Datang^_^\nJangan Nakal Ya..\n" + cl.getContact(op.param2).displayName + "\nWelcome to\n" + str(ginfo.name))
	          print "MEMBER JOIN TO GROUP"
        if op.type == 15:
          if wait["welcome"] == True:
            if op.param2 in admin:
                return
            cl.sendText(op.param1,cl.getContact(op.param2).displayName + "\nSee you Next Time kka....😘")
